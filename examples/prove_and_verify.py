...
spec = lf.find_zk_spec(credential["system"], credential["circuit_hash"])
if spec is None:
...
circuit = lf.generate_circuit(spec)
if lf.circuit_id(circuit) != spec.circuit_hash:
...
mdoc = bytes.fromhex(credential["mdoc_hex"])
timestamp = datetime.fromisoformat(credential["timestamp"])

proof = lf.prove(circuit, mdoc, issuer_pk, transcript, attrs, timestamp, spec)
print(f"proved: {len(proof)} bytes")

# verify() returns None on success and raises VerifierError otherwise.
lf.verify(circuit, issuer_pk, transcript, attrs, timestamp, proof, credential["doctype"], spec)
print("verified")
