...
from .conftest import _GOOGLE_DEVICE_NAMESPACES_XFAIL, CIRCUITS, PRESENTATIONS
...
_PROOF = next(pr for pr in _PRESENTATION.proofs if pr.prover == "isrg-rust")
...
@_GOOGLE_DEVICE_NAMESPACES_XFAIL
def test_google_cpp_verify_accepts_nonempty_device_namespaces(longfellow_for) -> None:
    p = _PRESENTATION
...
@pytest.mark.slow
def test_isrg_rust_verify_accepts_nonempty_device_namespaces(longfellow_for) -> None:
    p = _PRESENTATION
