...
def _verify(longfellow, case, proof: bytes) -> None:
    p = case.presentation
    longfellow.verify(
        p.issuer_pk,
        p.transcript,
...
def test_committed_proof(case: VerifyCase, longfellow_for):
    verifier = longfellow_for(case.verifier, case.circuit)
    proof = case.proof.path.read_bytes()
    _verify(verifier, case, proof)
    with pytest.raises(mdoc.VerifierError):
        _verify(verifier, case, _corrupted(proof))
...
def test_round_trip(case: RoundTripCase, longfellow_for):
    ...
    prover = longfellow_for(case.prover, case.circuit)
