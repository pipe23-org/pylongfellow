"""The binding's error type and the upstream error-code enums."""

from enum import IntEnum


class MdocProverErrorCode(IntEnum):
...
class MdocVerifierErrorCode(IntEnum):
...
ErrorCode = MdocProverErrorCode | MdocVerifierErrorCode | CircuitGenerationErrorCode


class LongfellowError(Exception):
    """Base class for errors raised by the binding.

    Attributes:
        code: The upstream return code, or None (e.g. for an unparseable circuit).
    """

    code: ErrorCode | None = None


class ProverError(LongfellowError):
    """A prover call failed; `code` is an `MdocProverErrorCode`."""

    def __init__(self, code: MdocProverErrorCode) -> None:
        self.code = code
        super().__init__(f"{code.name} ({code.value})")


class VerifierError(LongfellowError):
    """A verifier call failed; `code` is an `MdocVerifierErrorCode`."""

    def __init__(self, code: MdocVerifierErrorCode) -> None:
        self.code = code
        super().__init__(f"{code.name} ({code.value})")


class CircuitError(LongfellowError):
